...
TEST(Get, TypeLvalue) {
  ...
  using variant_const = rsl::variant<const int, long>;
  static_assert(std::same_as<rsl::variant_alternative<0, variant_const>::type, const int>);
  static_assert(std::same_as<rsl::variant_alternative<1, variant_const>::type, long>);
  static_assert(rsl::_variant_impl::variant_base<rsl::_impl::Storage<const int, long>>::alternatives
                    .get_index_of(^^int) == 0);  // TODO: should it be ^^const int ?
  static_assert(rsl::_variant_impl::variant_base<rsl::_impl::Storage<const int, long>>::alternatives
                    .get_index_of(^^long) == 1);
  {
    variant_const v(42);
